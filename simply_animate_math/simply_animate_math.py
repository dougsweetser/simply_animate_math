#!/usr/bin/env python
"""
usage: simply_animate_math.py [-s THING...] [-h]

Pulls images together to make an animation

optional arguments:
  -s, --skip THING...  skip 'plane' or 'd1' or 'd3' [default: None]
...
class SimplyAnimateMath:
    ...
    def run(self):
        ...
        if 'plane' not in self.skip:
            self.animate_plane(config.pure_numbers, "pure")
            self.animate_plane(config.qft_numbers, "qft", smosh_time=True)
    ...
    def write_time(self, time):
        ...
        for t_pos in template.time_pos.values():
            draw.text(t_pos, time_string, font=self.font, fill=(0, 0, 0, 255))

        del draw

        self.work_img.save("/tmp/time_123.png")

    def write_smosh_time(self):
        """Writes all time smoshed together."""

        draw = ImageDraw.Draw(self.work_img)

        for n in range(1, 7):
            for t_pos in template.time_pos.values():
                draw.text((t_pos[0] + 5 * n - 20,t_pos[1]), str(n), font=self.font, fill=(0, 0, 0, 255))
    ...
    def animate_plane(self, d2_numbers, short_name="test", \
            dir_name="Animations", smosh_time=False):
        ...
        for count, equation, operator, d2_number in \
            ...
            if smosh_time:
                self.write_smosh_time()

            self.work_img.save("{dn}/{sn}.{ct}.png".format(\
                    dn=dir_name, sn=short_name, ct=count))

            os.system("{convert} -loop 0 -delay 350 {dn}/{sn}.*.png {dn}/{sn}_{w}.gif".format(\
                    convert=config.convert, dn=dir_name, sn=short_name, w=config.width))
        print("gif done: {dn}/{sn}_{w}.gif".format(dn=dir_name, sn=short_name, w=config.width))
    ...
    def animate_plane_d1(self, d2_numbers, d1_number_numbers, \
        ...
        for equation, operator, d2_number, d1_number_number \
            ...
            os.system("{convert} -loop 0 -delay 60 {dn}/{sn}_{on}.*.png {dn}/{sn}_{on}_{w}.gif".format(\
                    convert=config.convert, dn=dir_name, \
                    sn=short_name, on=op_name, w=config.width))
            print("gif done: {dn}/{sn}_{on}_{w}.gif".format(\
                    dn=dir_name, sn=short_name, on=op_name, w=config.width))
    ...
    def animate_plane_d1_d3(self, d2_numbers, d1_number_numbers, \
        ...
        for equation, operator, d2_number, d1_number_number, \
            ...
            os.system("{convert} -loop 0 -delay 60 {dn}/{sn}_{on}.*.png {dn}/{sn}_{on}_{w}.gif".format(\
                    ...
                    on=op_name, w=config.width))
            print("gif done: {dn}/{sn}_{on}_{w}.gif".format(dn=dir_name, sn=short_name, on=op_name, w=config.width))


def merge_2_gifs(common_name, constant, operator_1, operator_2, dir_name="Animations"):
    """Merge two animations animations."""

    file_1 = "{dn}/{cn}_{con}_{op_1}_{w}.gif".format(\
            dn=dir_name, cn=common_name, con=constant, op_1=operator_1, w=config.width)
    file_2 = "{dn}/{cn}_{con}_{op_2}_{w}.gif".format(\
            dn=dir_name, cn=common_name, con=constant, op_2=operator_2, w=config.width)
    file_1_2 = "{dn}/{cn}_{con}_{op_1}_{op_2}_{w}.gif".format(\
            dn=dir_name, cn=common_name, con=constant, \
            op_1=operator_1, op_2=operator_2, w=config.width)
...
if __name__ == "__main__":
    ...
    SAM = SimplyAnimateMath(skip=ARGS['--skip'])
    SAM.run()
